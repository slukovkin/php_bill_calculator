...
$price = $data[count($data) - 1][1];

if ($counter_current > $counter_prev) {
  $sum = ($counter_current - $counter_prev) * $price;
  $check =  mysqli_query($db, "INSERT INTO `electro` (`id`, `counter_prev`, `counter_current`, `counter_acc`, `sum`, `data`) VALUES (NULL, '$counter_prev', '$counter_current', '$counter_current', '$sum', current_timestamp())");

  if ($check == 1) {
    // $_SESSION['message'] = $sum;
    header('Location: ../templates/electro.php');
  } else {
