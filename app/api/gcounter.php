...
if ($counter_current > $counter_prev) {
  ...
  $check =  mysqli_query($db, "INSERT INTO `gaz` (`id`, `counter_prev`, `counter_current`, `counter_acc`, `sum`, `data`) VALUES (NULL, '$counter_prev', '$counter_current', '$counter_current', '$sum', current_timestamp())");

  if ($check == 1) {
    // $_SESSION['message'] = $sum;
    header('Location: ../templates/gaz.php');
  } else {
